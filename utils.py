...
from my_dir import datasets_dir, results_dir

# get the save dir for the results
def get_save_results_dir(dataset_name, iid, alpha, num_clients, attack_method, defense_method):
    prefix_dir = results_dir[dataset_name]
    if iid == True:
        target_dir = prefix_dir +'/{}_iid-{}_numclients-{}_attack-{}_defense-{}_'.format(dataset_name, iid, num_clients, attack_method, defense_method)
    else:
        target_dir = prefix_dir +'/{}_iid-{}_alpha-{}_numclients-{}_attack-{}_defense-{}_'.format(dataset_name, iid, alpha, num_clients, attack_method, defense_method)

    import datetime
    target_dir += datetime.datetime.now().strftime('%Y%m%d%H%M%S') # date and time
    import uuid
    target_dir += str(uuid.uuid4())[:4] + '/' # 4bytes random string
    return target_dir
