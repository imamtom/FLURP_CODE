...
import argparse
# 从另一个py文件中导入数据集所在目录
from my_dir import datasets_dir
...
def run(attack_method, defense_method, num_clients, poisoned_client_portion, poison_data_portion, dataset_name, gpu_id, iid, alpha, global_rounds, local_epochs, local_learning_rate, local_momentum, batch_size, given_size, target_label):
    ...
    if dataset_name == "ImageNet12":
        all_dataset_train = ImagenetteDataset(root_dir = datasets_dir[dataset_name] + '/train')
        all_dataset_val = ImagenetteDataset(root_dir = datasets_dir[dataset_name] + '/val')

    elif dataset_name == "AGNews":
        all_dataset_train = AGNewsDataset(root_dir = datasets_dir[dataset_name] +'/train.csv')
        all_dataset_val = AGNewsDataset(root_dir = datasets_dir[dataset_name] +'/test.csv')
